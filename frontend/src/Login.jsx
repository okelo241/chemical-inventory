...
function Login({
  ...
  const handleGoogleSignIn = async () => {
    ...
    setLoading(true)
    try {
      // Persist workspace intent so App.jsx can apply personal vs org after redirect
      try {
        persistWorkspaceIntent(
          accountType === 'organization' ? 'organization' : 'personal',
          orgName || inviteOrgName || inviteOrgSlug || ''
        )
      } catch {
        /* ignore */
      }
      ...
      // Browser navigates to Google; if no URL, show hint
      if (!data?.url) {
      ...
      const hint =
        /provider is not enabled|Unsupported provider|validation_failed/i.test(
          ...
          ? ' Enable Google under Supabase → Authentication → Providers, and add this site to Redirect URLs.'
          : ''
      setError(msg + hint)
      setLoading(false)
    }
    // Keep loading true on success — page will redirect
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'login') {
      if (!canSubmitLogin) {
        setError(
          'Please enter a valid email and a password (8+ characters with a letter and a number).'
        )
        return
      }
    } else if (mode === 'signup') {
      ...
      if (!canSubmitReset) {
        if (!isRecoveryLink && !isOtpValid) {
          ...
          setError('New password must be 8+ characters with a letter and a number.')
      ...
      >
        ...
        {(mode === 'login' || mode === 'signup') && (
          <div className="auth-sso">
            ...
            <button
              type="button"
              className="auth-sso-google"
              onClick={handleGoogleSignIn}
              disabled={loading}
            >
              <svg className="auth-sso-google-icon" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <p className="auth-sso-hint">
              Works after Google is enabled in Supabase Auth. Same account email merges with password login when configured.
            </p>
          </div>
        )}
